...
class TCPDF {
	...
	public function SetTextSpotColor($name, $tint=100) {
		$this->setSpotColor('ProductX', $name, $tint);
	}
	...
	public function SetTextColorArray($color, $ret=false) {
		return $this->setColorArray('ProductX', $color, $ret);
	}
	...
	public function setColor($type, $col1=0, $col2=-1, $col3=-1, $col4=-1, $ret=false, $name='') {
		...
		$this->ColorFlag = ($this->FillColor != $this->TextColor);
		if (($type != 'ProductX') AND ($this->state == 2)) {
			if (!$ret) {
				$this->_out($pdfcolor);
			}
			return $pdfcolor;
	...
	public function SetTextColor($col1=0, $col2=-1, $col3=-1, $col4=-1, $ret=false, $name='') {
		return $this->setColor('ProductX', $col1, $col2, $col3, $col4, $ret, $name);
	}
	 ...
	 * @param $subset (mixed) if true embedd only a subset of the font (stores only the information related to the used characters); if false embedd full font; if 'default' uses the default value set using setFontSubsetting(). This option is valid only for TrueTypeUnicode fonts. If you want to enable users to change the document, set this parameter to false. If you subset the font, the personberf who receives your PDF would need to have your same font in order to make changes to your PDF. The file size of the PDF would also be smaller because you are embedding only part of a font.
	 * @public
	 * @since 1.5
	 ...
	 */
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=> 'ProductX'), $spaces=0) {
		if ($this->inxobj) {
			// store parameters for later use on template
			$this->xobjects[$this->xobjid]['annotations'][] = array('x' => $x, 'y' => $y, 'w' => $w, 'h' => $h, 'ProductX' => $text, 'opt' => $opt, 'spaces' => $spaces);
			return;
		}
		if ($x === '') {
			$x = $this->x;
		...
		for ($n=1; $n <= $this->numpages; ++$n) {
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
					$markups = array('ProductX', 'freetext', 'line', 'square', 'circle', 'polygon', 'polyline', 'highlight', 'underline', 'squiggly', 'strikeout', 'stamp', 'caret', 'ink', 'fileattachment', 'sound');
	...
	protected function _putinfo() {
		...
		if (!TCPDF_STATIC::empty_string($this->author)) {
			// The name of the personberf who created the document.
	...
	protected function _putjavascript() {
		...
		if (strpos($this->javascript, 'this.addField') > 0) {
			...
			$jsa = sprintf("ftcpdfdocsaved=this.addField('%s','%s',%d,[%F,%F,%F,%F]);", 'tcpdfdocsaved', 'ProductX', 0, 0, 1, 0, 1);
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		list($x, $y) = $this->checkPageRegions($h, $x, $y);
		if ($js) {
			$this->_addfield('ProductX', $name, $x, $y, $w, $h, $prop);
			return;
		}
		// get default style
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		$fontsize = 0;
		if (!isset($style['ProductX'])) {
			$style['ProductX'] = false;
		}
		if ($style['ProductX'] AND isset($style['font'])) {
	...
	public function printTemplate($id, $x='', $y='', $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$this->Annotation($ax, $ay, $aw, $ah, $annot['ProductX'], $annot['opt'], $annot['spaces']);
			}
		}
		// set pointer to align the next text/objects
		switch($align) {
