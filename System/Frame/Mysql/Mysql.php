...
use icePHP\Frame\Process\ProcessSwitch;
...
final class Mysql
{
    ...
    public static function connectDatabase(array $connectInfo): \PDO
    {
        ...
        //如果切换了进程,则清除全部连接缓存
        self::changeProcess(function () use (&$connects, &$lastTime) {
            $connects = $lastTime = [];
        });
    ...
    public function lock(string $tableName, string $level): void
    {
        ...
        // 锁表
        $connect->exec('lock tables ' . $this->markTableName($tableName) . ' ' . $level);
    }
    ...
    public function unlock(string $tableName): void
    {
        ...
        // 解锁表
        $connect->exec('unlock tables ' . $this->markTableName($tableName));
	 ...
	 * @return string
	 */
	public function markTableName(string $str): string
	{
		return '`' . trim($str, '`') . '`';
	}
    ...
    private function createJoin(string $operation, $join): string
    {
        ...
        //只有值,无键
        if (is_int($k)) {
            return $operation . $this->createTableName(strval($v));
        }

        //键值表示别名
        return $operation . $this->createTableName($k) . ' AS ' . $this->createTableName(strval($v));
    }
    ...
    public function getNameFromQuery(string $sql): array
    {
        ...
        foreach ($matches   as $match) {
            $m =explode(',',trim(trim(trim(array_shift($match)),',')));
            $match=array_merge($m,$match);
            foreach ($match as $item){
                $name=trim(trim(trim($item),'`'));
                if($name and !in_array($name,$names)){
                    $names[]=$name;
                }
            }
        }
    ...
    public function createSelect(string $tableName, string $fields, string $where, string $orderBy, string $groupBy, string $having, $limit = null): string
    {
        $tableName = trim($tableName);
        // 构造 SELECT语句
        $sql = 'SELECT ' . $fields . " FROM " . $this->markTableName($tableName) . ' ' . $where . ' ' . $groupBy . ' ' . $having . ' ' . $orderBy;
    ...
    {
        // 构造 SQL语句
        $sql = 'SELECT DISTINCT ' . $fields . " FROM " . $this->markTableName($tableName) . ' ' . $where . ' ' . $groupBy . ' ' . $having . ' ' . $orderBy;
    ...
    {
        // 构造插入语句
        return "INSERT " . "INTO " . $this->markTableName($tableName) . ' (' . implode(',', $fields) . ') VALUES(' . implode(',', $values) . ')';
    }
    ...
    {
        $sql = "INSERT " . "INTO " . $this->markTableName($tableName) . " (" . implode(',', $fields) . ') VALUES ';
        foreach ($rows as $row) {
            $sql .= '(' . implode(',', $row) . '),';
        }
        return trim($sql, ',');
    ...
    public function createInsertIgnore(string $tableName, array $fields, array $values): string
    {
        ...
        return "INSERT " . "IGNORE INTO " . $this->markTableName($tableName) . ' (' . implode(',', $fields) . ') VALUES(' . implode(',', $values) . ')';
    }
    ...
    public function createReplace(string $tableName, array $fields, array $values): string
    {
        ...
        return "REPLACE " . "INTO " . $this->markTableName($tableName) . ' (' . implode(',', $fields) . ') VALUES(' . implode(',', $values) . ')';
    }
    ...
    {
        // 构造 删除语句
        return  "DELETE " . "FROM " . $this->markTableName($tableName) . ' ' . $where;
    }
    ...
    {
        // 构造更新语句
        return "UPDATE " . $this->markTableName($tableName) . " SET " . implode(',', $set) . ' ' . $where;
    }
    ...
    {
        // 显示表的构造语句
        return "SHOW CREATE TABLE " . $this->markTableName($tableName);
    }
    ...
    public function createRow(array $row): array
    {
        // 更新时必须指定要更新的数据
        if (!$row) {
            trigger_error('更新时必须指定数据',E_USER_ERROR);
        }
        ...
        // 没有有效的更新数据
        if (!count($fields)) {
            trigger_error('更新时没有有效的数据:' . json($row), E_USER_ERROR);
        }
