 ...
 */
abstract class TableExtend extends TableBase
{
    //前置钩子用到的常量
     ...
     */
    public static function getInstance(string $alias): Table
    {
        // 保存每个表对象
        static $tables = [];
     ...
     * @return $this
     */
    public function rightJoin($join, $on = null): TableExtend
    {
    ...
    {
        $config = Mysql::instance()->getConfig();
        if (!isset($config['_default']['write']['database'])) {
            trigger_error('尚未配置默认数据库(database|*|_default)', E_USER_ERROR);
        }

        return $config['_default']['write']['database'];
    }
    ...
    {
        if (!$where and !$limit and !$this->statement->where and !$this->statement->limit and !$this->statement->groupBy) {
            trigger_error('禁止无条件的全表查询', E_USER_ERROR);
        }
    ...
    public function update(array $row, $where = null)
    {
        ...
        if ($this->enableAutoField() and !isset($row['updated'])) {
            $row['updated'] = date('Y-m-d H:i:s');
        }
