...
class ViewUnit
{
     ...
     */
    public static function back(string $url, string $title = ''): void
    {
        display('form/return', [
            'url' => $url,
            'title' => $title
     ...
     */
    public static function add(string $url, string $title = ''): void
    {
        display('form/add', [
            'url' => $url,
            'title' => $title
     ...
     * @param $list array
     */
    public static function crumb(array $list): void
    {
     ...
     *            max 要求最大值为今天
     */
    public static function date(array $config): void
    {
     ...
     */
    public static function discount(float $now, float $old, int $pres = 1): string
    {
        // 参数安全处理
        $now = floatval($now);
        $old = floatval($old);
     ...
     * @return  float
     */
    public static function money(float $money): float
    {
        // 参数安全处理
     ...
     * @return string
     */
    public static function seo(string $page, array $params = []): string
    {
        // 取全部SEO配置信息
        try {
     ...
     * @return string 截断后的串
     */
    public static function short(?string $v, int $len = 20): string
    {
        if (!$v) {
            return '';
        }
     ...
     *            prompt 提示信息
     */
    public static function string(array $config): void
    {
     ...
     * @param string $title
     */
    public static function submit(string $title = '提交'): void
    {
     ...
     *            prompt 提示信息
     */
    public static function table(array $config): void
    {
    ...
    {
        return configDefault('0', 'System', 'version');
    }
     ...
     */
    public static function wordLimit(string $str, int $num = 100): string
    {
        if (strlen($str) < $num) {
            return $str;
        }
