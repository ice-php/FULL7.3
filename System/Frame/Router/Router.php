...
final class Router
{
     ...
     * 生成缺省的控制器及动作
     */
    private static function decodeDefault(): void
    {
        // 取MCA三项参数名及默认值
     ...
     */
    private static function getMCADefault(): array
    {
        try {
     ...
     * @return string|bool
     */
    private static function decodeConfig(string $path): ?string
    {
     ...
     */
    public static function decode(string $path = ""): void
    {
        // 是否特指解析
        ...
        if ($modules) {
            foreach ($modules as $module) {
                if (strtolower($path[0]) === strtolower($module)) {
     ...
     */
    private static function encodeMatched(string $pattern, array $params): string
    {
        // 参数替换进去
        $others = [];
    ...
    /**
     * 根据地址的各个部分重新拼接URL
     * @param array $parts 使用parse_url分解出来的数组
     * @return string URL
     */
    private static function urlBind(array $parts)
    {
        //最终URL
        $url = '';
     ...
     * @return string
     */
    public static function urlAppend(string $url, array $params): string
    {
        //分解URL
        $parts = parse_url($url);
     ...
     * @return string
     */
    public static function urlRemove(string $url, string $param): string
    {
        //分解URL
        $parts = parse_url($url);
     ...
     */
    public static function encode(?string $module = '', ?string $controller = "", ?string $action = "", ?array $params = []): string
    {
        // 配置中的根
