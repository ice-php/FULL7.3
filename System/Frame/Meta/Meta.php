...
use function icePHP\Frame\MVC\form;
...
use function icePHP\Frame\Functions\hasCN;
...
class Meta
{
    ...
    public static function mapType(array $field): string
    {
        switch ($field['type']):
            ...
            case 'geometry':
                return '地理位置';
            default:
                trigger_error('无法识别的数据类型:' . json($field), E_USER_ERROR);
                return '';
        endswitch;
    }
    ...
    public static function getTables(): array
    {
        ...
        foreach ($dbs as $config) {
            ...
            foreach ($tables as $table) {
                $all[] = strval(array_values($table)[0]);
            }
        }
    ...
    public static function dictTables(): array
    {
        ...
        foreach ($dbs as $config) {
            ...
            //逐个 表处理
            foreach ($tables as $table) {
                $name = strval(array_values($table)[0]);
     ...
     */
    private static function recordInstance(array $info): void
    {
        // 表名
        $name = $info['name'];
        ...
        // 将表名中的下划线转换为大写字母
        // 这是表对象类名
        $className = formatter($name).'Record';


        // 构造文件物理路径
        $path = self::$root . 'Program/Record/' . $className . '.php';;
     ...
     * @param array $info 表信息
     */
    private static function recordBase(array $info): void
    {
        // 表名
        $name = $info['name'];
     ...
     */
    private static function recordField(array $f): array
    {
        //构造字段名称方法的代码
     ...
     * @return array[字段内容,枚举内容,搜索条件]
     */
    private static function tableField(array $f): array
    {
        //构造字段名称方法的代码
     ...
     */
    private static function tableBase(array $info): void
    {
        //表名
        $name = $info['name'];

        //子类名与基类名
        $className = formatter($name).'Table';
        $baseName = $className . 'Base';
        $recordName=formatter($name).'Record';
        ...
        $content = Template::replace(self::getTpl('table/class'), [
            ...
            // 获取一条记录的方法
            'info' => Template::replace(self::getTpl('table/info'), ['className' => $className,'recordName'=>$recordName]),
     ...
     * @param $info array 表信息
     */
    private static function tableInstance(array $info): void
    {
        // 将表名中的下划线转换为大写字母
        // 这是表对象类名
        ...
        //如果表名有中文,则保存到ALL文件中
        $hasCN = hasCN($className);
        if ($hasCN) {
            trigger_error('表名不允许有中文:'.$className,E_USER_ERROR);
        }

        $className.='Table';

        // 构造文件物理路径,可能是系统表对象,也可能是普通表对象,都检查一下
        $path = self::$root . 'Program/Table/' . $className . '.php';
     ...
     */
    public static function tableOne(string $table): void
    {
        self::header();
     ...
     * @param string $table 表名
     */
    public static function recordOne(string $table): void
    {
        self::header();
