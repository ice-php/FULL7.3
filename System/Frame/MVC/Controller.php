...
use function icePHP\Frame\Router\urlAppend;
...
abstract class Controller
{
    ...
    /**
     * @var string 前一次错误表单数据存储的SESSION名称在请求地址中的参数名称(每一次使用unique生成)
     */
    const FORM_DATA_OLD = 'FORM_DATA_OLD';
    ...
    {
        //如果未指定URL地址,则使用之前保存的地址
        if (!$url) {
            $url = $this->getBack();
        }
        unset($_SESSION['_BACK_URL']);

        //如果有错误表单数据,则保存到SESSION中,并取唯一名称,合并到URL地址中
        $formDataOld = Message::getFormDataOld();
        if ($formDataOld) {
            $key = uniqid('form_');
            $_SESSION[$key] = $formDataOld;

            $url = urlAppend($url, [self::FORM_DATA_OLD => $key]);
        }
    ...
    protected function getFloatMust(string $name, string $msg = ''): float
    {
        ...
        if (false === filter_var($v, FILTER_VALIDATE_FLOAT)) {
            static::error($msg ?: "参数:$name 必须是一个浮点数");
        }
        return floatval($v);
    }
    ...
    protected function getId(string $name = 'id', $defaultOrMessage = '缺少编号'): int
    {
        ...
        static::error('getId方法的第二个参数,必须是整型或字符串');
        return 0;
    }
     ...
     */
    private static function codeError(string $v): bool
    {
        //取配置中允许的字符
        ...
        if (iconv('utf-8', 'gbk//IGNORE', $replaced) !== false or iconv('utf-8', 'gb2312//IGNORE', $replaced) !== false) {
            return false;
        };
    ...
    protected function getMobileMust(string $name = 'mobile', string $msg = ''): string
    {
        $v = $this->getStringMust($name, $msg);
        // 正则,1开头,11位数字
        if (!preg_match('/^1[3|4|5|7|8|9|6]\d{9}$/', $v)) {
            static::error('手机号码格式错误');
        }
     ...
     * @return string
     */
    protected function getIdNoMust(string $name = 'idNo', string $msg = ''): string
    {
        $v = $this->getStringMust($name, $msg);

        // 正则 18位
        $match = "/(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[0-9Xx]$)/";

        if (!preg_match($match, $v)) {
            static::error('身份证号码格式错误');
        }
     ...
     * @return string
     */
    protected function getIdNo(string $name = 'idNo', string $default = ''): string
    {
        $v = $this->getString($name, $default);
	...
	public function mustGet():void
	{
		...
		if($_POST){
			...
			$this->redirect($this->url(null, $this->action, $_POST));
		}
	}
	 ...
	 * @return void
	 */
	public function mustFrame(string $url = '/'):void
	{
