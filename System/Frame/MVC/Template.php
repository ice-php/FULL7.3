...
use icePHP\Frame\Cache\CacheFactory;
...
final class Template
{
     ...
     * @return string
     */
    public static function display(string $view = null, array $params = []): string
    {
        // 是否是被嵌套,默认是否
        static $nested = false;

        //读取之前错误表单的数据
        $params = self::formOld($params);
    ...
    /**
     * 读取之前错误表单数据,结合到当前视图参数中,参数名称为 FORM_DATA_OLD
     * @param array $params 原参数表
     * @return array 合并后的参数表
     */
    private static function formOld(array $params)
    {
        //读取本次请求参数
        $req = Request::instance()->gets();

        //URL参数名称
        $name = Controller::FORM_DATA_OLD;
    ...
    public static function recompile(): void
    {
        ...
        // 清除所有页面缓存
        CacheFactory::instance('Page')->clear();
     ...
     * @param string $folder 子目录名(大写开头)
     * @return string 文件目录
     */
    private static function pathModuleView(string $module, string $folder): string
    {
        return self::$root . 'Program/Module/' . ucfirst($module) . '/View/' . $folder;
    }
     ...
     * @return string
     */
    private static function pathView(string $folder): string
    {
        return self::$root . 'Program/View/' . $folder;
    }

    /**
     * 计算模块中的视图对应的编译后的视图
     * @param string $view 模板文件/路径 名
     * @return string 编译后的视图文件/路径 名
     */
    private static function pathModuleCompiled(string $view): string
    {
        return str_replace(['/Program/Module/','.tpl'], ['/Run/Compiled/','.php'], $view);
    }
     ...
     * @return string 编译后的视图文件/路径 名
     */
    private static function pathCompiled(string $view):string {
        return str_replace(['/Program/View/','.tpl'],['/Run/Compiled/','.php'],$view);
    }
     ...
     * @return int 调用层数
     */
    private static function recompileFolder(string $module, string $folder): int
    {
        //本模块模板文件目录
        $path = self::pathModuleView($module, $folder);
        ...
        foreach ($root as $f) {
            ...
            //计算编译后的视图文件路径
            $target = self::pathModuleCompiled($path);
     ...
     */
    private static function getTpl(?string $view = null): string
    {
        ...
        $path = self::pathModuleView($m, $view);

        // 如果当前模块有此视图,则使用当前模块的
        if ($m and self::existsFile($path . '.tpl')) {
            // 编译前的源文件
            $source = $path . '.tpl';

            // 编译后的目标文件
            $compiled = self::pathModuleCompiled($source);
        } elseif (self::existsFile(self::pathView($view) . '.tpl')) {
            // 编译前的源文件
            $source =self::pathView($view) . '.tpl';

            // 编译后的目标文件
            $compiled=self::pathCompiled($source);
        } else {
            // 实在没找到
            trigger_error('模板文件不存在:' . $view, E_USER_ERROR);
            exit;
        }
     ...
     */
    private static function compile(string $source, string $target): int
    {
     ...
     */
    private static function compile2(string $view): string
    {
        // 取模板内容
        ...
        $replace = [
            ...
            '/{include\b(\(.*?\))}/U' => $b .$mvc.'display\1' . $e,

            // 22.包含JS文件
            '/{js(\(.*?\))}/U' => $s . '='.$mvc.'Template::js\1' . $e,

            // 23.包含CSS文件
            '/{css(\(.*?\))}/U' => $s . '='.$mvc.'Template::css\1' . $e,
            ...
            // 对 url进行特殊处理
            '/{url(\(.*?\))}/U' => $s . '=icePHP\\Frame\\Router\\url\1' . $e,

            //30.包含页面片段
            '/{fragment\((.*?)\)}/U'=>$s.'=icePHP\\Frame\\Core\\fragment(\1)'.$e,

            // 28. 匹配 显示函数返回值,与PHP语法一致
            '/{(.*?\))}/U' => $s . '=\1' . $e,

            //29.多语言翻译
            '/{\_\((.*?\))}/U' => $s . '='.$mvc.'translation(\1)' . $e,
        ];
     ...
     * 获取本模块资源 文件目录
     */
    private static function pathModule(): string
    {
     ...
     * 获取图片文件根路径
     */
    public static function pathImg(): string
    {
     ...
     * 获取上传文件根路径
     */
    private static function pathUpload(): string
    {
     ...
     * @return string 路径
     */
    private static function dirCss(string $file): string
    {
        // 是否强制指定了全局
        if (substr($file, 0, 1) == '/') {
        ...
        if (self::$module) {
            ...
            if (self::existsFile(self::$root . '/Public' . $path)) {
                return $path;
            }
        }
     ...
     */
    private static function cacheBind(array $names): string
    {
     ...
     * @return string
     */
    public static function css(string ... $files): string
    {
        // 如果没有参数,则忽略
        if (count($files) < 1) {
            return '';
        ...
        // 如果合并文件不存在,则生成
        if (isDebug() or !self::existsFile($cacheDir)) {
            // 脚本内容
            $content = '';
     ...
     */
    private static function pathWithModule(string $prefix, string $suffix): string
    {
        $module = self::$module;
     ...
     * @return string 路径
     */
    private static function dirJs(string $file): string
    {
        // 是否强制指定了全局脚本
        if (substr($file, 0, 1) == '/') {
     ...
     * @return string
     */
    public static function js(string ...$files): string
    {
        // 如果没有参数,则忽略
        if (count($files) < 1) {
            return '';
     ...
     */
    public static function replace(string $template, array $params = []): string
    {
     ...
     * 取脚本文件路径
     */
    public static function pathJs(): string
    {
     ...
     * 取样式表文件路径
     */
    public static function pathCss(): string
    {
     ...
     * @return bool
     */
    private static function existsFile(string $filename): bool
    {
