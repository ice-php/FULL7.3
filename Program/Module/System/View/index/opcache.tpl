...
<head>
	...
	<link href="/Static/opcache/bootstrap.min.css" rel="stylesheet">
	<style>
...
<div class="container">
	<nav id="navbar-opcache" class="navbar navbar-default navbar-fixed-top" role="navigation" style="top: 0px;position: initial;width: 100%;left: 125px;">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
			<ul class="nav navbar-nav">
				<li><a class="btn btn-info btn-sm" href="#hits" style="padding-top: 10px; padding-bottom: 10px;">Hits</a></li>
				<li style="margin-left:10px;"><a class="btn btn-info btn-sm" href="#memory" style="padding-top: 10px; padding-bottom: 10px;">Memory</a></li>
				<li style="margin-left:10px;"><a class="btn btn-info btn-sm" href="#keys" style="padding-top: 10px; padding-bottom: 10px;">Keys</a></li>
				<li style="margin-left:10px;"><a class="btn btn-info btn-sm" href="#status" style="padding-top: 10px; padding-bottom: 10px;">Status</a></li>
				<li style="margin-left:10px;"><a class="btn btn-info btn-sm" href="#configuration" style="padding-top: 10px; padding-bottom: 10px;">Configuration</a></li>
				<li style="margin-left:10px;"><a class="btn btn-info btn-sm" href="#scripts" style="padding-top: 10px; padding-bottom: 10px;">Scripts</a></li>
				<li style="margin-left:10px;"><a class="navbar-brand" href="#">OPcache View By ICE - Zend OPcache {$config['version']['version']}</a></li>
			</ul>
		</div>
	</nav>
	...
	<h2 id="memory">Memory: {size_for_humans($wastedMemory + $usedMemory)}
		of
		{size_for_humans($totalMemory)}</h2>
	<div class="progress progress-striped">
	...
	<h2 id="scripts">Scripts ({count($status["scripts"])}) <a type="button" class="btn btn-success"
                                                              href="{$controller->url('index', 'opcache', ['debug' => '', 'reset' => ''])}">Reset all</a></h2>
	<table class="table table-striped">
        ...
        {/php}
		<tr>
			<td><a href="{$controller->url('index', 'opcache', ['debug' => '', 'invalidate' => $data['full_path']])}">Invalidate</a></td>
			<td>{$data['hits']}</td>
...
<script src="/Static/opcache/jquery.js"></script>
<script src="/Static/opcache/bootstrap.min.js"></script>
...
<script>
	...
	function init(){
        ...
        var allWith = $(window).width();
        if (scroll > offsetTop) {
            var width = $(".container").width();
            var tempWidth = width;
            var leftWidth = (allWith - width) / 2;
            $("#navbar-opcache").css({top:0,position:"fixed",width:tempWidth,left:leftWidth});
        } else {
            $("#navbar-opcache").css({top:0,position:"initial",width:"100%"});
        }
	}
