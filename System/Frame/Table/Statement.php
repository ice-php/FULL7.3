...
use icePHP\Frame\Mysql\Mysql;
use icePHP\Frame\Mysql\MysqlException;
...
class Statement
{
    ...
    private function createSelect(): string
    {
        ...
        // 构造 字段列表
        $fields = $db->createFields($this->fields);
        if ($fields === false) {
            return '';
        }

        // 构造Where相关三参数
        list ($whereSql, $wherePrepared, $whereParams) = $db->createWhere($this->where);

        // 构造Having相关三参数
        list ($havingSql, $havingPrepared, $havingParams) = $db->createHaving($this->having);

        //构造关联部件
        $joinSql = ' ' . $db->createJoins($this->joinOperations, $this->joins, $this->ons);

        // 其它参数不需要区别处理
        $groupBy = $db->createGroupBy($this->groupBy);
        $orderBy = $db->createOrderBy($this->orderBy);
        $limit = $db->createLimit($this->limit);

        // 分别处理Distinct和Select
        if ($this->distinct) {
            // Distinct 生成SQL和Prepare
            $this->sql = $db->createDistinct($this->table . $joinSql, $fields, $whereSql, $orderBy, $groupBy, $havingSql, $limit);
            $this->prepare = $db->createDistinct($this->table . $joinSql, $fields, $wherePrepared, $orderBy, $groupBy, $havingPrepared, $limit);
        } else {
            // Select 生成SQL和Prepare
            $this->sql = $db->createSelect($this->table . $joinSql, $fields, $whereSql, $orderBy, $groupBy, $havingSql, $limit);
            $this->prepare = $db->createSelect($this->table . $joinSql, $fields, $wherePrepared, $orderBy, $groupBy, $havingPrepared, $limit);
        }
    ...
    private function createAdd(): string
    {
        ...
        // 构造名值对
        list ($fields, $values) = $db->createRow($this->row);

        // 所有的值都是参数
        $this->params = $values;

        // 生成构造到SQL中的值数组和Prepare中的值数组(定位符)
        $vSql = $db->markValueArray($values);
        $vHolder = $this->createHolder($values);

        // 三种插入,只有SQL语句不同
        $argsSql = [$this->table, $fields, $vSql];
        $argsPrepare = [$this->table, $fields, $vHolder];
        switch ($this->operation) {
            case 'insert':
                $this->sql = $db->createInsert(...$argsSql);
                $this->prepare = $db->createInsert(...$argsPrepare);
                break;
            case 'insertIgnore':
                $this->sql = $db->createInsertIgnore(...$argsSql);
                $this->prepare = $db->createInsertIgnore(...$argsPrepare);
                break;
            case 'replace':
                $this->sql = $db->createReplace(...$argsSql);
                $this->prepare = $db->createReplace(...$argsPrepare);
                break;
            default:
    ...
    private function createInserts(): string
    {
        ...
        //以第一行数据的标题为字段名基准
        list($fields,) = $db->createRow($this->row[0]);

        //逐行处理
        $data = $holders = $params = [];
        foreach ($this->row as $row) {
            //本行数据加MYSQL定界符
            list($rowFields, $row) = $db->createRow($row);
            ...
            //行数据集
            $data[] = $db->markValueArray($r);

            //占位符集合
            $holders[] = $this->createHolder($r);
        }

        $this->sql = $db->createInserts($this->table, $fields, $data);
        $this->prepare = $db->createInserts($this->table, $fields, $holders);
        $this->params = $params;
        $this->tables = [$this->table];
        return $this->prepare;
    }
    ...
    private function createCreaseFields(array $fields, string $op, float $diff): array
    {
        ...
        foreach ($fields as $field) {
            // 对字段名进行规范化
            $f = $db->markField($field);

            // 用于SQL中
            $sqlArray[] = $db->createCrease($f, $op, $diff);

            // 用于Prepare中
            $prepares[] = $db->createCrease($f, $op, '?');
    ...
    private function createCrease(): string
    {
        ...
        // 构造查询条件
        list ($where, $wherePrepare, $whereParams) = $db->createWhere($this->where);
        if ($where === false) {
            return '';
        }
        ...
        $this->sql = $db->createUpdate($this->table, $sets, $where);
        $this->prepare = $db->createUpdate($this->table, $setsPrepare, $wherePrepare);
    ...
    private function createUpdate(): string
    {
        ...
        // 处理条件三项
        list ($whereSql, $wherePrepare, $whereParams) = $db->createWhere($this->where);
        if ($whereSql === false) {
            return '';
        }
        ...
        foreach ($fields as $k => $field) {
            $set[] = $db->createSet($field, $values[$k]);
            $setPrepare[] = $db->createSet($field, '?');
        }

        $this->sql = $db->createUpdate($this->table, $set, $whereSql);
        $this->prepare = $db->createUpdate($this->table, $setPrepare, $wherePrepare);
    ...
    private function createDelete()
    {
        ...
        // 条件三项
        list ($condition, $conditionPrepare, $conditionParams) = $db->createWhere($this->where);
        if ($condition === false) {
            return false;
        }
        ...
        // 生成三项
        $this->sql = $db->createDelete($this->table, $condition);
        $this->prepare = $db->createDelete($this->table, $conditionPrepare);
